(function () {
  ...
  const media = manifest.items.map(item => Object.assign({}, item, {
    ...
    label: (item.type === "image" ? "Ảnh" : "Video") + " · " + dateLabel(item.date) + " · " + ageAt(item.date)
  ...
  function daysFor(items) {
    ...
    return Array.from(groups).sort(([a], [b]) => a.localeCompare(b)).map(([date, list]) => ({
      ...
      mediaLabel: countLabel(list), story: "Ngày " + dateLabel(date) + ", Thúy Yên được " + ageAt(date) + "."
    }));
  }
  function attach(scene, items, featured) {
    scene.galleryMedia = items;
    scene.timelineDays = daysFor(items);
    scene.galleryLabel = (scene.sensitive ? "Mở nhật ký theo dõi riêng · " : "Xem album · ") + countLabel(items);
    const dates = scene.timelineDays;
    if (dates.length) scene.dateRange = dateLabel(dates[0].date) + " → " + dateLabel(dates[dates.length - 1].date);
    const eligible = scene.sensitive ? [] : items.filter(item => approvedPreview.has(item.src) && !unsafe.has(item.src));
  ...
  if (health.length) {
    ...
    const scene = { id: "thuy-yen-health-digestion-library", month: monthAt(health[0].date), order: 65,
      type: "health-story", layout: "health-story", align: "center", sensitive: true, sourceRole: "user-confirmed",
      title: "Nhật ký theo dõi tiêu hóa", description: "Những ảnh và video gia đình lưu riêng để theo dõi tiêu hóa của Thúy Yên. Mở nhật ký để xem theo ngày; các hình ảnh này không xuất hiện trên hành trình chung." };
    attach(scene, health);
    data.scenes.push(scene);
    ["nguu-10m-constipation-story", "nguu-10m-bloody-stool-story", "nguu-health-constipation-2022"].forEach(id => {
      const entry = data.scenes.find(s => s.id === id);
      if (entry) entry.galleryTarget = scene.id;
    });
  }
  const groups = new Map();
  media.forEach(item => {
  ...
  Array.from(groups).sort(([a], [b]) => a - b).forEach(([month, items], index) => {
    ...
    const age = month < 12 ? month + " tháng" : Math.floor(month / 12) + " tuổi" + (month % 12 ? " " + month % 12 + " tháng" : "");
    const scene = { id: "nguu-" + String(month).padStart(2, "0") + "m-media-diary-v2", month, order: 70,
      ...
      subtitle: countLabel(items), description: relevant.length ? Array.from(new Set(relevant.map(o => o.story))).join(" ") :
        "Những khoảnh khắc của Thúy Yên từ " + dateLabel(items[0].date) + " đến " + dateLabel(items[items.length - 1].date) + ". Mỗi ngày trong album đều có tuổi của bé ở thời điểm ấy.",
