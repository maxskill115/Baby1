(function () {
  ...
  document.getElementById("birthDate").textContent = formatDate(profile.birthDate);
  const birthParts = profile.birthDate.split("-");
  document.getElementById("birthDay").textContent = birthParts[2];
  document.getElementById("birthMonthYear").innerHTML = "Tháng " + birthParts[1] + "<br>" + birthParts[0];
  document.getElementById("birthLunar").textContent = profile.birthLunar || "Chưa bổ sung";
  document.getElementById("birthLunarLabel").textContent = profile.birthLunarLabel || "Âm lịch";
  document.getElementById("birthTime").textContent = profile.birthTime || "Chưa bổ sung";
  ...
  document.getElementById("currentAge").textContent = formatAge(currentMonth);
  ...
  function buildContents() {
    ...
    contents.innerHTML = '<div class="journey-contents__heading"><strong>Hành trình của Yên</strong><button type="button" aria-label="Đóng mục lục">×</button></div><div class="journey-contents__list"></div>';
