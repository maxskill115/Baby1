        ...
        '<div class="baby-hero__shell">' +
          '<section class="baby-hero__identity" aria-label="Giới thiệu bé">' +
            ...
            '<div class="baby-hero__identity-row">' +
              '<div class="baby-hero__avatar" aria-hidden="true"><span id="profileInitials">TY</span></div>' +
              '<div class="baby-hero__title-block">' +
                '<h1 class="baby-hero__name" id="profileName">Thúy Yên</h1>' +
          ...
          '<aside class="baby-hero__memory-card" aria-label="Hồ sơ chào đời">' +
            '<div class="memory-card__heading">' +
              '<p class="memory-card__kicker">Hồ sơ chào đời</p>' +
              '<span class="memory-card__seal" id="profileSeal" aria-hidden="true">TY</span>' +
            '</div>' +
            '<div class="memory-card__date">' +
              '<strong id="birthDay">04</strong><span id="birthMonthYear">Tháng 04<br>2021</span>' +
            '</div>' +
            '<div class="memory-card__facts">' +
              '<div class="memory-card__fact">' +
                '<span class="memory-card__icon" aria-hidden="true">◇</span>' +
                '<div class="memory-card__copy"><small>Dương lịch</small><span id="birthDate">04/04/2021</span></div>' +
              '</div>' +
              '<div class="memory-card__fact">' +
                '<span class="memory-card__icon" aria-hidden="true">☾</span>' +
                '<div class="memory-card__copy"><small id="birthLunarLabel">Âm lịch</small><strong id="birthLunar"></strong></div>' +
              '</div>' +
              '<div class="memory-card__fact">' +
                '<span class="memory-card__icon" aria-hidden="true">◷</span>' +
                '<div class="memory-card__copy"><small>Giờ sinh</small><strong id="birthTime"></strong></div>' +
              '</div>' +
              '<div class="memory-card__fact">' +
                '<span class="memory-card__icon" aria-hidden="true">✦</span>' +
                '<div class="memory-card__copy"><small>Hiện tại</small><span id="currentAge"></span></div>' +
              '</div>' +
            '</div>' +
          '</aside>' +
        '</div>' +
